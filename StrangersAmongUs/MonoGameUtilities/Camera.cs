...
namespace MonoGameUtilities
{
    public class Camera
    {
        ...
        //Height and width of each single-cell sprite's floor area.
        public int SpriteWidth { get; set; }
        public int SpriteHeight { get; set; }

        //Height and width of the map in Cell units.
        ...
        {
            var cameraPosition = new Vector2((cell.X - cell.Y) * SpriteWidth,
               (cell.X + cell.Y) * SpriteHeight);
            var cameraCenteredOnTilePosition =
               new Vector2(cameraPosition.X + 168 / 2, // + SpriteWidth / 2
                   cameraPosition.Y + 208 / 2);
            if (clampToMap)
            {
        ...
        {
            var cameraMin = new Vector2(-(ViewportWidth / Zoom / 2) - MapHeight * SpriteWidth,
                ViewportHeight / Zoom / 2);

            var cameraMax = new Vector2(MapWidth * SpriteWidth +
                (ViewportWidth / Zoom / 2),
                (MapHeight + MapWidth) * SpriteHeight +
                (ViewportHeight / Zoom / 2));
